...
export default function Login() {
  ...
  async function submit(event) {
    ...
    try { const response = await fetch('/api/auth/login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }); const data = await response.json(); if (!response.ok) throw new Error(data.message); sessionStorage.setItem('portfolio_session', data.token); setMessage(`Bienvenue, ${data.user.name}.`); }
